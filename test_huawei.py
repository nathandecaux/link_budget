...
window = Tk()
window.title("Link Budget Huawei")
window.geometry()
...
def freqEvent(event):
    ...
    for row in table:
        freq = row['BAND_DESIGNATOR']
        if cb1.get()=='RTN900' and cb0.get() == 'Yes':
            mod_card =str(row['MODEL']).split('_')[-2]
        elif cb1.get()=='RTN900' and cb0.get() == 'No':
...
lbl = Label(equiFr, text="Modem Card + ODU")
...
am = ttk.Combobox(equiFr,values = ['Yes','No'])
am.grid(column=1, row=17)
