...
if __name__ == '__main__':
        ...
        def bkapp2(doc):
            ...
            if(checks[0]):
                ...
                def update_data(event):
                    source1.data = plt.ColumnDataSource(
                        data=graph.plotRain(g1a, g1b, el, geoloc, rrS.value, tau, p0, xpic, equip, freqS.value, card, bw, ref_mod)).data
            ...
            if(checks[1]):
                ...
                graph2 = plt.figure(title='Capacity according to the distance',
                                    x_axis_label='Distance (km)', y_axis_label='Capacity (Mbps)',tooltips=TOOLTIPS)
                ...
                def update_data2(event):
                    ...
                    source2.data = plt.ColumnDataSource(
                        data=graph.plotMod(g1a,g1b, el, geoloc,
                                  rrS2.value,float(polarS.value), p0,xpicS.value, equipS.value, float(freqS.value), cardS.value, float(bwS.value), ref_mod)).data
                ...
                def add_data2(event):
                    ...
                    curr_dat = graph.plotMod(g1a,g1b, el, geoloc,
                                  rrS2.value,float(polarS.value), p0, xpicS.value, equipS.value, float(freqS.value), cardS.value, float(bwS.value), ref_mod)
    ...
    def scenarEric():
        ...
        if form.is_submitted():
            ...
            [eb_stab,ex_tab,e_mw_tab,mw_stab,mw_xtab] = poubelle_2.getScenarii()
